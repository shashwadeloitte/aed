...
import { X, AlertCircle, Brain, Wrench, CheckCircle, Clock, User, MapPin, Activity, Download, ThumbsUp, ThumbsDown, MessageSquare, FileText } from "lucide-react";
...
export default function AbendDetailModal({ abend, open, onClose, defaultTab = "overview" }: AbendDetailModalProps) {
  ...
  const getConfidenceLevel = (score: number) => {
    if (score >= 85) return { level: 'High', color: 'text-green-600', bgColor: 'bg-green-50' };
  ...
  function getRootCause(abendType: string): string {
    ...
    return causes[abendType as keyof typeof causes] || 'Unknown system error requiring investigation.';
  }
  ...
  function getRecommendedAction(abendType: string): string {
    ...
    return actions[abendType as keyof typeof actions] || 'Perform system analysis and restart with default parameters.';
  }
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div>
                    ...
                    <p className="text-sm font-medium">{abend.id}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <Badge className={cn("ml-2", 
                      abend.status === 'resolved' ? 'bg-green-100 text-green-800' : 
                      abend.status === 'PENDING_MANUAL_APPROVAL' ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800'
                    )}>
                  ...
                  <div>
                    ...
                    <Badge className={cn("ml-2", 
                      abend.priority === 'high' ? 'bg-red-100 text-red-800' : 
                      abend.priority === 'medium' ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100 text-gray-800'
                    )}>
                ...
                <CardContent>
                  <div className="space-y-4">
                    {workflowSteps.map((step, index) => {
                      ...
                      return (
                        <div key={step.state} className="flex items-center space-x-3">
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      ...
                      <span>{Math.round(detailedAbend.durationInState)} min</span>
                    </div>
                    <div className="flex justify-between text-sm">
                ...
                <CardContent>
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      <Badge className={cn(confidence.bgColor, confidence.color)}>
                        {confidence.level} ({abend.confidence}%)
                      </Badge>
                    </div>
                    <Progress value={abend.confidence} className="h-2" />
                    <p className="text-sm text-gray-600">
                      The AI system has analyzed the abend patterns and historical data to provide diagnosis and remediation recommendations.
                    </p>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-4">
                    ...
                    <div>
                      <h4 className="font-medium text-gray-900">Return Code Identified</h4>
                      <p className="text-sm text-gray-600 mt-1">GG33</p>
                    </div>
                    <div>
                      <h4 className="font-medium text-gray-900">Root Cause Summary</h4>
                      <p className="text-sm text-gray-600 mt-1">
                        Invalid record format in input file during STEP1 of job OGGLOAD
                      </p>
                    </div>
                    <div>
                      <h4 className="font-medium text-gray-900">Detailed Explanation</h4>
                      <div className="text-sm text-gray-600 mt-1 space-y-2">
                        <p>1. The job OGGLOAD (JOB12456) started successfully at 23:58:45.</p>
                        <p>2. STEP1 of the job began loading data into DB2 table XYZ.TXN_LOG at 23:58:46.</p>
                        <p>3. At 23:58:50, an error was reported: "INVALID RECORD FORMAT IN INPUT FILE".</p>
                        <p>4. This error led to job termination at 23:58:51 with abend code GG33.</p>
                        <p>5. The abend reason is specified as "DATA FORMAT ERROR ON LINE 2457".</p>
                        <p>6. Additional information from STC12345 confirms the abend occurred in module OGGEXTR1 at offset X'00001A24'.</p>
                        <p>7. Message OGG9999I indicates termination due to a fatal input validation error.</p>
                      </div>
                    </div>
                    <div>
                      <h4 className="font-medium text-gray-900">IMS Status Code Interpretation</h4>
                      <p className="text-sm text-gray-600 mt-1">
                        GG33: User abend code indicating a data format error in Oracle GoldenGate processing
                      </p>
                    </div>
                  </div>
                </CardContent>
                    ...
                    <div className="space-y-4">
                      <div className="p-4 bg-blue-50 rounded-lg">
                        <div className="flex items-center justify-between mb-2">
                          <h4 className="font-medium text-blue-900">Primary Recommendation</h4>
                          <Badge className="bg-blue-100 text-blue-800">Recommended</Badge>
                        </div>
                        <p className="text-sm text-blue-800 mb-3">
                          Send email to operations Team to Restart the JOB from the Abended State and also include the step number (STEP1)
                        </p>
                        <div className="flex items-center space-x-4">
                          <div className="text-xs text-blue-600">
                            Success Rate: <span className="font-medium">{abend.confidence}%</span>
                          </div>
                          <div className="text-xs text-blue-600">
                            Est. Duration: <span className="font-medium">15 min</span>
                          </div>
                        </div>
                      </div>

                      <div className="space-y-3">
                        <h4 className="font-medium text-gray-900">Corrective Actions</h4>
                        
                        <div className="p-4 border rounded-lg bg-gray-50">
                          <div className="space-y-3">
                            <div>
                              <div className="flex items-center justify-between">
                                <span className="text-sm font-medium">Review and correct the input file format</span>
                                <div className="text-xs text-gray-500">Priority: High</div>
                              </div>
                              <p className="text-xs text-gray-600 mt-1">
                                Review and correct the format of the input file, particularly around line 2457.
                              </p>
                            </div>

                            <div>
                              <div className="flex items-center justify-between">
                                <span className="text-sm font-medium">Verify Oracle GoldenGate extract format</span>
                                <div className="text-xs text-gray-500">Priority: High</div>
                              </div>
                              <p className="text-xs text-gray-600 mt-1">
                                Verify the expected input format for the Oracle GoldenGate extract process.
                              </p>
                            </div>

                            <div>
                              <div className="flex items-center justify-between">
                                <span className="text-sm font-medium">Check input file generation process</span>
                                <div className="text-xs text-gray-500">Priority: Medium</div>
                              </div>
                              <p className="text-xs text-gray-600 mt-1">
                                Check if any recent changes were made to the input file generation process.
                              </p>
                            </div>

                            <div>
                              <div className="flex items-center justify-between">
                                <span className="text-sm font-medium">Update OGGEXTR1 module</span>
                                <div className="text-xs text-gray-500">Priority: Medium</div>
                              </div>
                              <p className="text-xs text-gray-600 mt-1">
                                Ensure the OGGEXTR1 module is up-to-date and compatible with the current input format.
                              </p>
                            </div>

                            <div>
                              <div className="flex items-center justify-between">
                                <span className="text-sm font-medium">Rerun job from STEP1</span>
                                <div className="text-xs text-gray-500">Priority: Final</div>
                              </div>
                              <p className="text-xs text-gray-600 mt-1">
                      ...
                      <div className="space-y-3">
                        <h4 className="font-medium text-gray-900">Explainability</h4>
                        <div className="p-3 bg-gray-50 rounded-lg">
                          <p className="text-sm text-gray-700 mb-2">This analysis is based on the following key factors:</p>
                          <ul className="text-xs text-gray-600 space-y-1">
                            <li>• Clear identification of job name (OGGLOAD) and job ID (JOB12456) from the JES2 log</li>
                            <li>• Specific error messages indicating invalid record format and data validation error</li>
                            <li>• Abend code GG33 with reason "DATA FORMAT ERROR ON LINE 2457"</li>
                            <li>• Additional context from STC12345 log entries pinpointing the module and offset of the error</li>
                            <li>• The sequence of events in the log, showing normal job start followed by error and abend</li>
                            <li>• Recognition of Oracle GoldenGate-specific messages and processes</li>
                          </ul>
                          <p className="text-xs text-gray-600 mt-2">
                            The recommendations are derived from standard troubleshooting practices for data format errors in mainframe batch jobs, particularly those involving Oracle GoldenGate extract processes.
                          </p>
                        </div>
                      </div>
                    </div>
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="flex items-start space-x-3 p-3 bg-gray-50 rounded-lg">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between">
                        <span className="font-medium text-sm">ABEND_REGISTERED</span>
                        <span className="text-xs text-gray-500">{new Date(abend.timestamp).toLocaleString()}</span>
                      </div>
                      <p className="text-sm text-gray-600 mt-1">
                  ...
                  <div className="flex items-start space-x-3 p-3 bg-gray-50 rounded-lg">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between">
                        <span className="font-medium text-sm">LOG_EXTRACTION_INITIATED</span>
                        <span className="text-xs text-gray-500">{new Date(new Date(abend.timestamp).getTime() + 30000).toLocaleString()}</span>
                      </div>
                      <p className="text-sm text-gray-600 mt-1">
                  ...
                  <div className="flex items-start space-x-3 p-3 bg-gray-50 rounded-lg">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between">
                        <span className="font-medium text-sm">LOG_UPLOAD_TO_S3</span>
                        <span className="text-xs text-gray-500">{new Date(new Date(abend.timestamp).getTime() + 120000).toLocaleString()}</span>
                      </div>
                      <p className="text-sm text-gray-600 mt-1">
                  ...
                  <div className="flex items-start space-x-3 p-3 bg-gray-50 rounded-lg">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between">
                        <span className="font-medium text-sm">PREPROCESSING_LOG_FILE</span>
                        <span className="text-xs text-gray-500">{new Date(new Date(abend.timestamp).getTime() + 150000).toLocaleString()}</span>
                      </div>
                      <p className="text-sm text-gray-600 mt-1">
                  ...
                  <div className="flex items-start space-x-3 p-3 bg-blue-50 rounded-lg">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between">
                        <span className="font-medium text-sm">AI_ANALYSIS_INITIATED</span>
                        <span className="text-xs text-gray-500">{new Date(new Date(abend.timestamp).getTime() + 180000).toLocaleString()}</span>
                      </div>
                      <p className="text-sm text-gray-600 mt-1">
                        AI analysis started
                      </p>
                      <p className="text-xs text-gray-500 mt-1">
                        Confidence: {abend.confidence}% • Abend Type: {abend.abendType}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start space-x-3 p-3 bg-green-50 rounded-lg">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between">
                        <span className="font-medium text-sm">REMEDIATION_SUGGESTIONS_GENERATED</span>
                        <span className="text-xs text-gray-500">{new Date(new Date(abend.timestamp).getTime() + 240000).toLocaleString()}</span>
                      </div>
                      <p className="text-sm text-gray-600 mt-1">
                        AI remediation suggestions generated
                      </p>
                      <p className="text-xs text-gray-500 mt-1">
                        Status: {ABEND_STATUS_CONFIG[abend.status].label} • Assigned To: {abend.assignedTo}
                      </p>
                    </div>
                  </div>
                  ...
                  {abend.status === "PENDING_MANUAL_APPROVAL" && (
                    <div className="flex items-start space-x-3 p-3 bg-yellow-50 rounded-lg">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center justify-between">
                          <span className="font-medium text-sm">PENDING_MANUAL_APPROVAL</span>
                          <span className="text-xs text-gray-500">{new Date(new Date(abend.timestamp).getTime() + 300000).toLocaleString()}</span>
                        </div>
                        <p className="text-sm text-gray-600 mt-1">
                    ...
                    <div className="flex items-start space-x-3 p-3 bg-green-50 rounded-lg">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-xs text-gray-500">{new Date(new Date(abend.timestamp).getTime() + 360000).toLocaleString()}</span>
                        </div>
                        <p className="text-sm text-gray-600 mt-1">
...
*${abend?.jobId || 'IYM0798'}  02 01 1*            '${abend?.jobId || 'IYM0798'}  02 01 14/12/24 03:55 AC67766  IYMP009890'.   000000
*4/12/24 03:55 AC*
*67766  IYMP00989*
*0               *
                    77  FILLER                      PIC X(040) VALUE
*${abend?.jobId || 'IYM0798'}  WORKING*      '${abend?.jobId || 'IYM0798'}  WORKING STORAGE STARTS HERE '.
* STORAGE STARTS *
*HERE            *
...
*${abend?.jobId || 'IYM0798'}--       *      05  WS-PFX                  PIC X(10) VALUE '${abend?.jobId || 'IYM0798'}--'.
